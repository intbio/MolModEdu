...
def main():
	...
	pdb=parsePDB('../analysis_data/only_nucl_init.pdb') # get PSF - to be able to get atomic names
	
	pdb_init=parsePDB('../analysis_data/only_nucl_init.pdb')
	pdb_aver=parsePDB('../analysis_data/only_nucl_average.pdb')
	dcd.setAtoms(pdb.select(dna_backbone)) # this will determine what set to use for superposition and covariance
...
#Histone folds
	...
	print "Num atoms:", len(pdb.select(dna_backbone).getIndices())
