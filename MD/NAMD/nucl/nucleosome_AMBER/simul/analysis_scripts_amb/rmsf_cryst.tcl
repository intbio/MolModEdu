#Here we do RMSF, but using crystal as reference
...
proc rmsf_cryst {selection first last ref} {
...
#Here is an option to use average structure as a
# reference - just to check the correctness
########################################
...
set v1 [lindex $veclist $i]
set v2 [lindex $ref_coord $i]
#we are cheating here, by initializing the ref_coord list not with 0, but with crystal
set vn [vecadd $v1 $v2 ]
set ref_coord [lreplace $ref_coord $i $i $vn] 
}
}
 for {set i 0} {$i < $num_at} {incr i} {
 set v2 [lindex $ref_coord $i]
 set vs [vecscale $v2 [expr 1.0 / ($last - $first + 2)]]
set ref_coord [lreplace $ref_coord $i $i $vs] 
 	}
###############
...
set rmsf [rmsf_cryst $all 990 1000 0]

$all set beta $rmsf
$all writepdb ../analysis_data/nucl_rmsf_cryst.pdb
...
set outfile [open ../analysis_data/rmsf_cryst_chains.dat w]
