...
####!!!!Correct PDB so that there is a space before B-factor
			...
			if(r.match(line)):
				...
				if(not re.match('^CA$|^C$|^O$|^N$|^P$|^OP1$|^OP2$|^C1\'$|^C2\'$|^C3\'$|^C4\'$|^C5\'$|^O3\'$|^O4\'$|^O5\'$',atomname)):
					...
					if((resid==resid_old) or (mean==0)):
						...
						mean=mean/numat
						exp_r[chainid][0].append(resid_old)
						exp_r[chainid][1].append(mean)
	...
	for key in exp_r:
		...
		exp_rn[key][1,:]=(exp_rn[key][1,:]/(8**0.5)/3.14)
	...
	# plt.show()
	fig.savefig(file.replace('.dat','_exp_sch.png'),dpi=(200))
