...
df=rename(df,c("Chain"="PROT_chain","Resid"='PROT_resid'))
...
h3dataE=melt(h3dataE,id.vars=c('PROT_resid'),measure.vars=c('E'),variable.name='PROT_chain',value.name='sasa_fract')

h3data=arrange(h3data,PROT_chain,PROT_resid,desc(sasa_fract))

head(h3data)

a<-ggplot(data=h3data,aes(x=PROT_resid,y=(100-sasa_fract*100),fill=PROT_chain,color=PROT_chain))+
...
geom_point()+scale_y_continuous(limits=c(-95,100),breaks=seq(0,100,by=20.0),labels=seq(0,100,by=20.0),expand=c(0,0))+
...
ylab("Buried SASA, %")+#ggtitle("H3, contacts of side chains with bases in minor grooves")+#geom_point(data=data.frame(PROT_resid=100,tot_num=1),color='red',fill='red')+
annotation_custom(h3, ymin=-85, ymax=0, xmin=0.5,xmax=135.5)


ggsave("../analysis_data/pub_prot_sasa_h3.png",plot=a,height=2.5,width=12)
