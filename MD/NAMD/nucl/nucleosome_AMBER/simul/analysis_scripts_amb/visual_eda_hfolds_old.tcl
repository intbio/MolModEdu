...
# mol addfile ../analysis_data/nucl_rmsf.pdb 
display rendermode GLSL
# mol load psf 1kx5nt_ready.psf
# mol addfile md_nucl_solv.dcd step 1 first 0 last 1000  waitfor all


set handle [nmwiz load ../analysis_data/eda_hfolds.nmd]
mol modstyle 0 0 Tube 0.4 10
...
mol modstyle  0 2 Tube 0.4 10
mol modmaterial  0 2 AOEdgy


#rotate x by -70

#display depthcue on
 set ::nmdset0::cone_radius 0.5
 set ::nmdset0::cylinder_radius 0.3
 ::nmdset0::drawArrows
...
# #add some text on new layer
 ...
 draw text " $txtx [expr $txty-($txtlncount*$txtstep)] 0 " "Essential dynamics in nucleosome" size 1.5 thickness 3
 incr txtlncount 2
...
add_text_layer DNA
draw color 6
draw text " $txtx [expr $txty-($txtlncount*$txtstep)] 0 " "Histone folds core - 1st principal mode" size 1.5 thickness 3
incr txtlncount 1
...
#render snapshot ../analysis_data/nucl_rmsf.tga
...
exec rm -f ../analysis_data/eda_hfolds_1.mov
exec ffmpeg -i dat/%d.tga -s 1000x750 -q:v 0 -pix_fmt yuv420p  ../analysis_data/eda_hfolds_1.mov
