...
#############################################################################################
# requieres: input_param.tcl aa_hydro_scale.tcl colorbar.tcl add_text_layer.tcl
#
#
#ffmpeg -i %d.dat.tga -s 1000x750 -q:v 0 -pix_fmt yuv420p  movie.mov
# vmd tcl script: shows nucleosome 1KX5 structure in different representations and angles, renders 2800 frames in dat/ folder.
#RMSD for modes is 1: 4.9; 2: 3.00; 3:2.8; 4:2.5
...
mol load psf ../analysis_data/only_dna_p.psf
# mol addfile ../../analysis_data/only_nucl_init.pdb
# mol addfile ../../analysis_data/md_nucl.dcd  waitfor all step 50
mol addfile ../analysis_data/eda_dnabb_m${MODE}.dcd  waitfor all


# mol addfile ../../analysis_data/md_nucl_solv.dcd last 1000 step 10 waitfor all
...
mol representation Tube 1.5 10
mol color ColorID 6
mol selection {all}
mol material AOEdgy
mol addrep top
...
# #add some text on new layer
 ...
 draw text " $txtx [expr $txty-($txtlncount*$txtstep)] 0 " "MD simulations of NCP with tails and linkers (1000 ns)" size 1.5 thickness 3
 incr txtlncount 1
 draw text " $txtx [expr $txty-($txtlncount*$txtstep)] 0 " "DNA backbone essential mode ${MODE}, VIEW $VIEW" size 1.5 thickness 3
 incr txtlncount 2
...
# mol modmaterial 8 0 AOEdgy
	...
	display update
	material change opacity AOEdgy 1.0
...
add_text_layer TIME
draw color 0
set time [format "RMSD: %5.1f A" [expr abs( $MAXRMSD *2* ($i-$nframes/2)/$nframes) ]]
...
#}
 render snapshot dat/$i.dat.tga
  ...
  for {set i $nframes} {$i >= 0} {incr i -1} {
animate goto $i
...
#mol showrep 0 8 off
#}
set k [expr 2*$nframes - $i]
 render snapshot dat/$k.dat.tga
 	}

# mol modstyle 0 0 NewCartoon 0.350000 20.000000 2.630000 0
# mol modstyle 1 0 NewCartoon 0.350000 20.000000 2.630000 0
# mol modstyle 2 0 NewCartoon 0.350000 20.000000 2.630000 0
# mol modstyle 3 0 NewCartoon 0.350000 20.000000 2.630000 0
...
mol drawframes 1 0 $ff:$fs:$nframes
mol drawframes 1 1 $ff:$fs:$nframes
mol drawframes 1 2 $ff:$fs:$nframes
mol drawframes 1 3 $ff:$fs:$nframes
mol drawframes 1 4 $ff:$fs:$nframes
mol drawframes 1 5 $ff:$fs:$nframes
mol drawframes 1 6 $ff:$fs:$nframes
mol drawframes 1 7 $ff:$fs:$nframes

render snapshot ../analysis_data/eda_dnabb_${MODE}_view_${VIEW}_overlayed.tga



exec rm -f ../analysis_data/eda_dnabb_${MODE}_view_${VIEW}.mov
exec ffmpeg -i dat/%d.dat.tga -s 1000x750 -q:v 0 -pix_fmt yuv420p  ../analysis_data/eda_dnabb_${MODE}_view_${VIEW}.mov
