...
ann_num_inner=ddply(subset(dna_prot_avr_m,DNA_resid<39.5),c('PROT_chain','variable'),summarize,avr_sum=round(sum(value),0))
t=ddply(subset(dna_prot_avr_m,DNA_resid<39.5),c('variable'),summarize,avr_sum=round(sum(value),0))
t$PROT_chain='ALL'
...
ann_num_outer=ddply(subset(dna_prot_avr_m,(DNA_resid>39.5)&(DNA_resid<73.5)),c('PROT_chain','variable'),summarize,avr_sum=round(sum(value),0))
t=ddply(subset(dna_prot_avr_m,(DNA_resid>39.5)&(DNA_resid<73.5)),c('variable'),summarize,avr_sum=round(sum(value),0))
t$PROT_chain='ALL'
...
scale_y_continuous(limits=c(0,46),breaks = c(0,10,20,30,40),labels=c('','','20','','40'),name='Contacts',expand=c(0,0))+
...
ggsave(filename="../analysis_data/pub_int_dna_prot_avr_cryst_av_core.png",plot=q,width=15,height=2.0)
