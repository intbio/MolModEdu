...
# library(gridExtra)
##############
FREQ=1 #number of data points in 1 ns
...
t=seq(length(a))/FREQ
res=nls(a~exp(-t/tau),start=list(tau=1),control=c(maxiter=500))
return(coef(res)[[1]])
}
}
...
dna_all=dna_act
# r=dna[dna$BPnum==74,'Roll']
# rt=ts(r,frequency=10)
# a=acf(rt,plot=FALSE)$acf
# x=seq(length(a))
# res=nls(a~exp(-x/tau),start=list(tau=1))
# coef(res)[[1]]

# qnorm((1 + 0.95)/2)/sqrt(x$n.used)
...
# dna_all=merge(dna1,dna_err,by=c('BPnum'),suffixes = c("_av","_er"))
