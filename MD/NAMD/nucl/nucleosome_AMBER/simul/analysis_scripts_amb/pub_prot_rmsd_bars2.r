...
head(h3data)
h3data_amore=subset(h3data,max_rmsd==6)
h3data_amore$color='amore'

a<-ggplot(data=h3data,aes(x=Resid,y=(max_rmsd),fill=color))+
...
annotation_custom(h3, ymin=-5, ymax=0, xmin=0.5,xmax=135.5)

ggsave("../analysis_data/pub_prot_max_rmsd_h3_2.png",plot=a,height=2,width=12)
####H4
...
head(h4data)
h4data_amore=subset(h4data,max_rmsd==6)
h4data_amore$color='amore'

a<-ggplot(data=h4data,aes(x=Resid,y=(max_rmsd),fill=color))+
# geom_tile(aes(fill=RMSD)) + 
geom_bar(stat='identity',position='identity')+scale_y_continuous(limits=c(-5,6),breaks=seq(0,6),labels=seq(0,6,by=1))+
...
xlab('')+
scale_color_manual(breaks=c('CA','bSCH','amore'),values=c('amore'='red','CA'='blue','bSCH'='green'),labels=c(expression(paste('C',alpha,'-atoms')),'Side chain','> 6Å      '),name='')+
geom_point(data=h4data_amore,aes(color=color),fill='red',size=2)+

# theme(plot.margin = unit(c(1,1,1,1), "cm"))+
ylab("RMSD, Å")+#ggtitle("RMSD, C-alpha, nucleosome core. Histone H3, A")+
annotation_custom(h4, ymin=-5, ymax=0.5, xmin=0.5,xmax=102.5)

ggsave("../analysis_data/pub_prot_max_rmsd_h4_2.png",plot=a,height=2,width=10)
...
head(h2adata)
h2adata_amore=subset(h2adata,max_rmsd==6)
h2adata_amore$color='amore'

a<-ggplot(data=h2adata,aes(x=Resid,y=(max_rmsd),fill=color))+
# geom_tile(aes(fill=RMSD)) + 
geom_bar(stat='identity',position='identity')+scale_y_continuous(limits=c(-5,6),breaks=seq(0,6),labels=seq(0,6,by=1))+
...
# theme(plot.margin = unit(c(1,1,1,1), "cm"))+
ylab("RMSD, Å")+#ggtitle("RMSD, C-alpha, nucleosome core. Histone H3, A")+
geom_point(data=h2adata_amore,aes(color=color),fill='red',size=2)+
annotation_custom(h2a, ymin=-5, ymax=0.5, xmin=0.5,xmax=128.5)

ggsave("../analysis_data/pub_prot_max_rmsd_h2a_2.png",plot=a,height=2.1,width=13)
...
head(h2bdata)
h2bdata_amore=subset(h2bdata,max_rmsd==6)
h2bdata_amore$color='amore'

a<-ggplot(data=h2bdata,aes(x=Resid+3,y=(max_rmsd),fill=color))+
# geom_tile(aes(fill=RMSD)) + 
geom_bar(stat='identity',position='identity')+scale_y_continuous(limits=c(-5,6),breaks=seq(0,6),labels=seq(0,6,by=1))+
...
# scale_y_continuous(limits=c(0,1210),breaks = round(seq(0,1000, by = 100),1))+
xlab('')+
geom_point(data=h2bdata_amore,aes(color=color),fill='red',size=2)+
# theme(plot.margin = unit(c(1,1,1,1), "cm"))+
ylab("RMSD, Å")+#ggtitle("RMSD, C-alpha, nucleosome core. Histone H3, A")+
annotation_custom(h2b, ymin=-5, ymax=0.5, xmin=0.5,xmax=125.5)

ggsave("../analysis_data/pub_prot_max_rmsd_h2b_2.png",plot=a,height=2,width=12)
