...
# m=merge(a$C,a$IP,by.x=c("ION_atom","ION_ind","INT_atom","INT_chain","INT_resid"),by.y=c("ION_atom","ION_ind","INT_atom","INT_chain","INT_resid"),all.x=TRUE,suffix=c('','y'))
# m2=merge(m,a$HB,by.x=c("ION_atom","ION_ind","INT_atom","INT_chain","INT_resid"),by.y=c("ION_atom","ION_ind","INT_atom","INT_chain","INT_resid"),all.x=TRUE,suffix=c('','z'))
...
phosphate<-c('O3\'','O5\'','OP1','OP2','P','O1P','O2P')
intbb<-c('C','O','N','CA')
...
if(resname %in% c('ADE','GUA','CYT','THY','DA','DT','DG','DC','DA3','DT3','DG3','DC3','DA5','DT5','DG5','DC5'))
{
...
if(!(df['INT_resname']%in%c('ADE','GUA','CYT','THY','DA','DT','DG','DC','DA3','DT3','DG3','DC3','DA5','DT5','DG5','DC5'))) {return('NA')}
if(df['INT_resname']%in%c('ADE','DA','DA3','DA5')) {
	if(df['INT_atom'] %in% c('N6','C6','C5','N7','C8')){ return('major')}
	else if(df['INT_atom'] %in% c('C2','N3','C4')){ return('minor')}
	else return('NA')}
if(df['INT_resname']%in%c('GUA','DG','DG3','DG5')) {
	if(df['INT_atom'] %in% c('O6','C6','C5','N7','C8')){ return('major')}
	else if(df['INT_atom'] %in% c('N2','C2','N3','C4')){ return('minor')}
	else return('NA')}
if(df['INT_resname']%in%c('THY','DT','DT3','DT5')) {
	if(df['INT_atom'] %in% c('C6','C5','C4','C7','C5M','O4')){ return('major')}
	else if(df['INT_atom'] %in% c('N3','C2','O2')){ return('minor')}
	else return('NA')}

if(df['INT_resname']%in%c('CYT','DC','DC3','DC5')) {
