...
set outfile [open ../analysis_data/ion_atm_sphere.dat w]	 
...
puts $outfile "Number of ions, charges and total charge within certain distance from nucleosome center"
...
for {set r 0.} { $r < 200.1 } { set r [expr $r + 1.0] } {
set np 0.
set nn 0.
set nph 0.
set nnh 0.
...
set sel [atomselect top "resname SOD and (sqr(x)+sqr(y)+sqr(z) < sqr($r))" frame $i]
set pc [$sel num]
$sel delete
set sel [atomselect top "resname CLA and (sqr(x)+sqr(y)+sqr(z) < sqr($r))" frame $i]
set nc [$sel num]
$sel delete

set sel [atomselect top "resname ARG LYS and name NZ CZ and (sqr(x)+sqr(y)+sqr(z) < sqr($r))" frame $i]
set ph [$sel num]
$sel delete

set sel [atomselect top "((resname GLU and name CD) or (resname ASP and name CG)) and (sqr(x)+sqr(y)+sqr(z) < sqr($r))" frame $i]
set nh [$sel num]
$sel delete

set sel [atomselect top "(nucleic) and name P  and (sqr(x)+sqr(y)+sqr(z) < sqr($r))" frame $i]
set ld [$sel num]
$sel delete
...
exec python2.7 myplot_ions.py ../analysis_data/ion_atm_sphere.dat 
