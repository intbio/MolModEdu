# we calculate contacts and visualize them
exec mkdir -p dat 
...
mol ssrecalc top
mol addfile ../analysis_data/md_nucl_solv.dcd first 990 waitfor all
...
# #add some text on new layer
 ...
 draw text " $txtx [expr $txty-($txtlncount*$txtstep)] 0 " "4A contacts in nucleosome" size 1.5 thickness 3
 incr txtlncount 1
# draw text " $txtx [expr $txty-($txtlncount*$txtstep)] 0 " "Isosurfaces for average number density of ions" size 1.5 thickness 3
 # incr txtlncount 2
...
#Let's find contacts between DNA an protein


mol representation VDW
mol color name
mol selection "noh and (same residue as (protein and (within 4 of (nucleic and noh)) and noh))"
...
render snapshot ../analysis_data/dna_prot_contacts_visual_4.tga
...
exec rm -f ../analysis_data/dna_prot_contacts_visual_4.mov
exec ffmpeg -i dat/%d.tga -s 1000x750 -q:v 0 -pix_fmt yuv420p  ../analysis_data/dna_prot_contacts_visual_4.mov
